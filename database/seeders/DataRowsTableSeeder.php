...
class DataRowsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        $dataType = Voyager::model('DataType')->where('slug', 'replace-keywords')->firstOrFail();
        ...
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'timestamp',
                'display_name' => __('joy-voyager-bread-replace-keyword::seeders.data_rows.deleted_at'),
                'required'     => 0,
                'browse'       => 0,
                'read'         => 0,
                'edit'         => 0,
        ...
        $dataRow = $this->dataRow($dataType, 'replace_keyword_belongsto_created_by_relationship');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'relationship',
        ...
        $dataRow = $this->dataRow($dataType, 'replace_keyword_belongsto_modified_by_relationship');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'relationship',
        ...
        $dataRow = $this->dataRow($dataType, 'replace_keyword_belongsto_assigned_to_relationship');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'relationship',
        ...
        $dataRow = $this->dataRow($dataType, 'replace_keyword_belongsto_parent_relationship');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'relationship',
                ...
                'delete'       => 0,
                'details'      => [
                    'model'       => Voyager::modelClass('ReplaceKeyword'),
