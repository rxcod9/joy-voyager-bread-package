...
class TranslationsTableSeeder extends Seeder
{
    ...
    private function replaceKeywordsTranslations()
    {
        ...
        //
        $cat = Voyager::model('ReplaceKeyword')->where('name', 'replace-keyword-1')->first();
        if ($cat->exists) {
        ...
        $cat = Voyager::model('ReplaceKeyword')->where('name', 'replace-keyword-2')->first();
        if ($cat->exists) {
    ...
    private function dataTypesTranslations()
    {
        ...
        $dtp = Voyager::model('DataType')->where($_fld, __('joy-voyager-bread-replace-keyword::seeders.data_types.category.singular'))->firstOrFail();
        if ($dtp->exists) {
        ...
        $_tpl = ['data_types', $_fld];
        $dtp  = Voyager::model('DataType')->where($_fld, __('joy-voyager-bread-replace-keyword::seeders.data_types.category.plural'))->firstOrFail();
        if ($dtp->exists) {
